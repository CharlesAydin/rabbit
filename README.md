...
Messages are serialized using [ProtocolBuffers](https://developers.google.com/protocol-buffers/?hl=en). ProtocolBuffers is simple, programming language-agnostic and efficient, which is important considering the message volume that Hopper consumes (billions of messages / day).
...
The following recommendations are designed to limit message traffic to the minimum required by the Hopper analytics engine:
  ...
  1. It is recommended that data providers trigger no more than **50 trips** per client request. In case of truncation, trips should be ordered according to price before truncating.
...
###TripBatch
The **TripBatch** message represents a batch of **Trip** messages that represent a single search result provided to a customer's query.
...
###Trip
The **Trip** message represents a single bookable itinerary shopped by a customer on a data provider's system. It can represent a one-way itinerary or a return itinerary.
...
stops        | integer  | no       | no         | The number of stops on this flight segment. Usually 0 unless the aircraft makes a stop, e.g. to refuel.
outgoing     | boolean  | no       | no         | Boolean flag indicating that this segment belongs to the outbound portion (**true**) or the return portion (**false**) of a roundtrip fare. This should be **false** for segments of a one-way fares.
